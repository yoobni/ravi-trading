/**
 * R42 — F6 신호 그대로, SL 빠듯 + TP 큰 변형.
 *
 * 가설: tight SL로 false break 빨리 잘라내고 큰 winner는 더 키움
 *
 * Variants:
 *   F6_BASE  : TP+5/SL-2/14d
 *   F6_v2 ★  : TP+7/SL-2.5/14d
 *   T1       : TP+8/SL-1.5/14d   (SL 더 빠듯)
 *   T2       : TP+10/SL-1.5/14d
 *   T3       : TP+10/SL-2/14d
 *   T4       : TP+12/SL-2/21d
 *   T5       : TP+15/SL-2/21d
...
const VARIANTS: Variant[] = [
  ...
  { name: 'F6_v2 (7/-2.5/14d) ★',   tp: 7,  sl: -2.5,  maxBars: 84 },
  { name: 'T1 TP8/SL-1.5/14d',      tp: 8,  sl: -1.5,  maxBars: 84 },
  { name: 'T2 TP10/SL-1.5/14d',     tp: 10, sl: -1.5,  maxBars: 84 },
  { name: 'T3 TP10/SL-2/14d',       tp: 10, sl: -2,    maxBars: 84 },
  { name: 'T4 TP12/SL-2/21d',       tp: 12, sl: -2,    maxBars: 126 },
  { name: 'T5 TP15/SL-2/21d',       tp: 15, sl: -2,    maxBars: 126 },
  { name: 'T6 TP15/SL-3/21d',       tp: 15, sl: -3,    maxBars: 126 },
  { name: 'T7 TP20/SL-2/30d',       tp: 20, sl: -2,    maxBars: 180 },
  { name: 'T8 TP20/SL-3/30d',       tp: 20, sl: -3,    maxBars: 180 },
];
...
(async () => {
  ...
  const L: string[] = [];
  L.push('='.repeat(170));
  L.push(`R42 빠듯 SL + 큰 TP — F6 신호 그대로, 1년 (2025-06~2026-06), ${barsByCoin.size}코인`);
  L.push('='.repeat(170));
  ...
  console.log(L.join('\n'));
  fs.writeFileSync(path.join(OUT_DIR, `${stamp}_R42_TIGHT_SL.txt`), L.join('\n'));
  process.exit(0);
})();
