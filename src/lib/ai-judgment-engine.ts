...
- confidence가 낮으면 반드시 hold(관망)을 선택합니다.
  ...
  input_schema: {
    ...
    properties: {
      ...
      confidence: {
        ...
        minimum: 0,
        maximum: 100,
        description: '신뢰도 (0~100). 설정된 minConfidence 미만이면 hold.',
      },
      compositeScore: {
        type: 'number',
        minimum: -100,
...
async function callClaude(
  ...
  // minConfidence 미달이면 강제 hold
  const minConfidence = getConfig().trading?.minConfidence ?? 40;
  if (response.confidence < minConfidence && response.action !== 'hold') {
    response.action = 'hold';
    response.reasoning += ` [신뢰도 ${response.confidence}% < 설정값 ${minConfidence}% → 관망으로 보정]`;
  }
